...
const Login = ({navigation}) => {
  ...
  try {
    ...
    const response = await axios.post(
     'https://a564-2405-201-100c-8057-d282-35dd-cbe9-3ee1.ngrok-free.app/api/v1/users/login',
      PayLoad,
    );
      ...
      if(!!response.data){
        await axios.get(API_CALL.getAllUsers)
              .then((resp) => {
                ...
                const filteredUserData = allUserData?.filter(user => user.id === response.data.userId);
                setTimeout(() => {
                  storeData(response.data.token, filteredUserData?.[0].id, filteredUserData?.[0].role, filteredUserData?.[0].email);
                }, 5000);                                
              })
              .catch(err => {
                  console.log(err, "3")
  ...
  return (
    <ScrollView>
      ...
      <SafeAreaView style={styles.mainContainer}>
        ...
        <Text style={[styles.inputHeading,{color : FocusInput === "username" ? 'red' : 'gray'}]}>Username</Text>
          <View style={styles.inputFieldWrapper}>
            <TextInput
              placeholder="johnsmith"
              placeholderTextColor={FocusInput === "username" ? 'red' : 'gray'}
              style={[styles.inputField, {textTransform:'lowercase', borderColor : FocusInput === "username" ? 'red' : 'gray'}]}
          ...
          <Text style={[styles.inputHeading,{color : FocusInput === "password" ? 'red' : 'gray'}]}>Password</Text>
          <View style={[styles.inputFieldWrapper, {
            ...
            <TextInput
              ...
              style={[styles.inputField, {borderColor : FocusInput === "password" ? 'red' : 'gray'}]}
              selectionColor={'gray'}
              name="password"
              returnKeyType="next"
